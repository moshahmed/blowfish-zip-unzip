...
# $Header: c:/cvs/repo/mosh/mbin/7zs.sh,v 1.85 2017-11-21 08:08:55 a Exp $
...
Options:
  ...
  -oldfile otpfile .. extract otp from otpfile and saves same otp in a new otpfile.
  -v=1           .. verbose
Setup
  keyfile=$keyfile
  pemfile=$pemfile
...
Example
  $CMD a archive.zip -r dir *.txt # pack using public key in pemfile, no passphrase needed.
...
while [ $# -gt 0 ]  ;do
  case $1 in
    ...
    -pemfile) shift; pemfile=$1 ; need_file $pemfile pemfile ;;
    -oldfile) shift; oldfile=$1 ; need_file $oldfile oldfile
      need_file $keyfile keyfile
      otp=$(base64 -d $oldfile | openssl pkeyutl -decrypt -inkey $keyfile ) 
      need_val "$otp" otp
      info "# In $oldfile found otp=$otp"
      ;;
    -v) verbose=1 ;;
    -v=*) verbose=${1#-*=} ;;
...
case $action in
  a) # Generate otp (one time password)
    ...
    need_file $otpfile
    info "# Encrypted opt with $pemfile to otpfile=$otpfile"
