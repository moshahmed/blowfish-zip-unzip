...
openssl genrsa -out $SSH_PRV_KEY 2048
# ssh-keygen -m PEM -t rsa -b 2048 -C "your_email@example.com" -N $PASSPHRASE -f $SSH_PRV_KEY
ssh-keygen -e -f $SSH_PRV_KEY -m PKCS8 > $SSH_PKS_KEY
...
openssl pkeyutl -decrypt -inkey $SSH_PRV_KEY -in $FILE_ENC -out $FILE_DEC

echo diff $FILE_DEC $FILE_TXT
diff $FILE_DEC $FILE_TXT

echo sign $FILE_TXT
openssl pkeyutl -sign -inkey $SSH_PRV_KEY -in $FILE_TXT -out $FILE_SIG
