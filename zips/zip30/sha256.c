/*
$Header: c:/cvs/repo/github/bfz/zips/zip30/sha256.c,v 1.1 2014-02-04 11:24:58 a Exp $
...
void sha256_continue(char *buf, int buflen) {
  sha256_update(&_ctx2, (uint8_t *) buf, buflen);
}


void sha256_end(char *hexit, int hexit_len) {
  unsigned char sha256sum[32];
  int j;
  sha256_finish(&_ctx2, sha256sum);
  for(j = 0; j < hexit_len/2; j++) {
...
void sha256_key(char *buf, char *salt, int salt_len, char* hexit, int hexit_len) {
  /* this returns sha256(buf) as 64 hex chars. */
  unsigned char sha256sum[32];
  ...
  sha256_update(&ctx, (uint8_t *) salt, salt_len);
...
local_function
char* sha256_bytes(char *buf, int buflen, char *hexit2, int hexit2size) {
  unsigned char sha256sum[32];
  int j;
  context_sha256_t ctx;
  ...
  sha256_starts(&ctx);
  sha256_update(&ctx, (uint8_t *) buf, buflen);
  sha256_finish(&ctx, sha256sum);
  for(j = 0; j < hexit2size/2; j++) {
      sprintf(hexit2 + j * 2, "%02x", sha256sum[j]);
  }
  hexit2[hexit2size-1]='\0';
  return hexit2;
}
...
int sha256_self_test(void) {
  ...
  for(i = 0; i < 3; i++) {
      if(i < 2) {
          sha256_bytes(sha_self_test_msg[i],
