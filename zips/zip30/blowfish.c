...
local_function
void bf_e_block(uint32_t *p_xl, uint32_t *p_xr) {
...
local_function
void bf_d_block(uint32_t *p_xl, uint32_t *p_xr) { // not used.
    uint32_t temp, xl = *p_xl, xr = *p_xr;
...
local_function
void bf_key_init(uint8_t *password, uint8_t *salt, int salt_len) {
    int i, j, keypos = 0;
    ...
    memset(key, 0, sizeof(key)); // clear_key
...
local_function
int bf_check_tables(uint32_t ipa[18],
                           uint32_t sbi[4][256],
                           uint32_t val) {
    int i, j;
    uint32_t c=0;
...
typedef struct {
    uint8_t password[64], salt[9], plaintxt[9], cryptxt[9], badcryptxt[9];
    uint32_t keysum;
} struct_bf_test_data;
...
int bf_self_test(void) {
    ...
    for (i=0; i<bn; i++) {
        bf_key_init(
            (uint8_t *) bf_test_data[i].password,
            (uint8_t *) bf_test_data[i].salt,
            (int)       strlen( (char*) bf_test_data[i].salt));
...
static int     randbyte_offset=0,
               update_offset=0;
static uint8_t ofb_buffer[BF_OFB_LEN]; /* 64 bytes */
...
void bf_ofb_init(const uint8_t *iv, int iv_len) {
    int i, mi;
    ...
    mi = get_max(iv_len, BF_OFB_LEN );
    for (i=0; i< mi; i++) {
        ofb_buffer[ i % BF_OFB_LEN ] ^= iv[i % iv_len ];
    }
}

void bf_ofb_update(uint8_t c) {
    ofb_buffer[ update_offset++ % BF_OFB_LEN ] ^= c;
}

uint8_t bf_ranbyte(void) {
...
#define printfr(x)  (void)0
#else
#define printfd(x)  (void)0
...
void hash_salt_pass(char *cryptkey, file_header *fh) {
  ...
  for(i=0; i<sha_key_strenghtening_rounds;i++) {
    sha256_continue((char*)&fh->salt.uc[0], sizeof(fh->salt.uc));
    sha256_continue(cryptkey, key_strlen);
  }	   
  sha256_end(hash, sizeof(hash));
